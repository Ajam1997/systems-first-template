...
Configuration:
  requirements/requirement-map.yml  — UN → FR/NFR/IF/KPM decomposition,
                                      stage → UN mapping
  config/stages.yml                  — milestone titles (fallback;
                                      preferred source is live GitHub
                                      milestone list, parsed by title)
...
def build_reverse_maps(issue_map: dict, req_map: dict) -> tuple[dict, dict, dict, dict]:
    """Return (req_to_uns, un_to_stage, stage_to_epic_num).
    ...
    for stage_num, stage_data in req_map.get("stages", {}).items():
        # Epic number lookup (legacy backward compat; Milestones supersede Epics).
        # issue_map["epics"] is keyed by "Stage N" short form.
        epic_entry = issue_map.get("epics", {}).get(f"Stage {stage_num}", {})
        if epic_entry:
...
def rollup(pr_body: str, dry_run: bool = False) -> None:
    ...
    req_to_uns, un_to_stage, stage_to_epic_num = build_reverse_maps(issue_map, req_map)

...
### B.2 — Install the marketplace plugins, then activate the profile

Before running `sf-init`, install the discipline-lead agents this
profile needs from the PHOTONFORGE marketplace (see the root
[`README.md`](../README.md#install-per-project)):

```bash
claude plugin marketplace add Ajam1997/PHOTONFORGE
claude plugin install systems-first-core@photonforge
claude plugin install systems-first-<discipline>@photonforge   # one per discipline this profile activates
...
```bash
sf-init --activate-profile <A|B|C> --skip-agents --dry-run
```

`--skip-agents` is required — this template no longer ships local
`.claude/agent-packs/` for `sf-init` to copy from; the plugins
installed above are the agents.
...
- Run `sf-docs` to fill AUTO sections
...
to add/remove leads (installing/uninstalling the matching
`systems-first-<discipline>` plugin), then re-run `sf-init
--skip-agents` without `--activate-profile`.
...
Active leads:     <list>
Render chain:     [ok] sf-docs  [ok] sf-kpm-rollup  [ok] sf-sysml
                  [ok] sf-artifacts
...
- **Status label moves** — `sf-pr-rollup` owns `verified` / `validated`. Claude does not touch them.
- **Hand edits inside AUTO sentinels** — regenerated by `sf-docs`. Edit the Issue, not the rendered table.
